...
public class RespondToReviewUseCase {
    ...
    public ReviewResponse respondToProductReview(Long reviewId, String respuesta) {
        ...
        if (!resena.isPublished()) {
            throw new BusinessRuleViolationException(
                "RESENA_NO_PUBLICADA",
                "No se puede responder a una reseña que no ha sido publicada");
        }

        resena.setRespuestaEmprendedor(respuesta);
        resena.setRespuestaFecha(Instant.now());

        ResenaProducto updated = resenaProductoRepository.save(resena);
        return reviewMapper.toReviewResponse(updated);
    }

    public ReviewResponse respondToTenantReview(Long reviewId, String respuesta) {
        Long tenantId = requireTenantId();
